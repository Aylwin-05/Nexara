...
async def websocket_endpoint(
    ...
    async with AsyncSessionLocal() as db:
        # Scope Row-Level Security for this socket. The after_begin
        # listener in database.py re-publishes this as the
        # transaction-local GUC app.current_user_id on EVERY
        # transaction, so it survives the db.commit() after each
        # event below. A plain set_config(..., true) set once here
        # would be wiped by the first commit, leaving later events
        # unscoped: verify_access() would then see no participant
        # rows and reject every relayed message with "Access denied."
