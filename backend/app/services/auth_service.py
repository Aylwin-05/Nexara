...
class AuthService:
    ...
    async def reset_two_fa(
        ...
        result = await self.verify_otp(
            email,
            otp,
            # 2FA recovery acts on an existing account, never a
            # registration; consents pass through so behavior is
            # unchanged for the (impossible) missing-user edge.
    ...
    async def verify_otp(
        ...
        *,
        ...
        # A NEW account cannot be created without both consents. Checked
        # before the OTP is consumed so a forgotten checkbox does not
        # burn the code; existing accounts never re-consent.
        if existing_user is None and not (privacy_consent and terms_consent):
            raise ValueError(
        ...
        user = User(
            ...
            terms_consent_at=datetime.now(UTC),
        )
