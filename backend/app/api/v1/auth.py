...
async def verify_otp(
    ...
    try:
        result = await service.verify_otp(
            request_body.email,
            request_body.otp,
            privacy_consent=request_body.privacy_consent,
            terms_consent=request_body.terms_consent,
        )

    except ValueError as error:
        # A brand-new account without both consents: refuse creation.
        raise HTTPException(
            status_code=400,
            detail=str(error),
        ) from error
...
@router.post(
    ...
    dependencies=[
        ...
        # ponytail: DEBUG-wide relaxation, keep strict when DEBUG=false.
        rate_limit(
            "auth.refresh.ip",
            600 if settings.DEBUG else 60,
            600,
        ),
    ],
)
